...
function JaxContestant()
{var loadingTimeout=400;var iframe;this.loadingFunction=function(){};

this.doneLoadingFunction=function(){};this.stringify=function(arg){var c,i,l,o,u,v;switch(typeof arg){case'object':if(arg){if(arg.constructor==Array){o='';for(i=0;i<arg.length;++i){v=this.stringify(arg[i]);if(o&&(v!==u)){o+=',';}
if(v!==u){o+=v;}}
...
return doc.body;}
//alert(comName);
this.buildXmlReq=function(comName,controler,task,postData){
	var xmlReq='';
	if(iframe){
		...
		//alert(func);
		xmlReq+='option=com_'+comName;xmlReq+='&task='+task;xmlReq+='&controller='+controler;
...
this.submitTask=function(comName,controler,task,postData){
	...
	xmlhttp.onreadystatechange=function(){
		if(xmlhttp.readyState==4){
			if(xmlhttp.status==200){
				jaxcontestant.doneLoadingFunction();
				jaxcontestant.processResponse(xmlhttp.responseText);
			}
			else{}
		}
	}
	var id=1;
	var xmlReq=this.buildXmlReq(comName,controler,task,postData);
...
this.call_contestants=function(comName,controler,task,postData){
	//alert(postData);
	var arg="";
	//alert(arg);
	...
	//alert(arg);
	this.submitTask(comName,controler,task,arg);
}
this.call_selectecard=function(comName,controler,task,postData){
	//alert(postData);
	this.submitTask(comName,controler,task,postData);
}
this.call_groupfilter=function(comName,controler,task,request_variable,postData){
	var arg = request_variable+'='+postData;
	this.submitTask(comName,controler,task,arg);
}
this.call=function(comName,sFunction){
	...
	if(arguments.length>2){
		for(var i=2;i<arguments.length;i++){
			...
			var t=new Array('_d_',encodeURIComponent(a));
			arg+="arg"+i+"="+this.stringify(t)+"&";
			}else{var t=new Array('_d_',encodeURIComponent(a));
			arg+="arg"+i+"="+this.stringify(t)+"&";}
			}}
...
var jaxcontestant=new JaxContestant();