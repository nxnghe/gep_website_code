 ...
 * @since		1.6
 */
class CandidatesControllerRegistration extends CandidatesController
{
	/**
	...
	public function register()
	{
		...
		$model	= $this->getModel('Registration', 'CandidatesModel');
	...
	function ajaxSetMessage(){
	   ...
	   $myLabel = ($txtLabel == 'Field') ? JText::_('PR_FIELD') : $txtLabel;

	   //echo $fieldName;exit;

       $langMsg = (empty($txtLabel)) ? $langMsg : $myLabel.'_'.$langMsg;
       if($fieldName == 'password2' ){
       	$langMsg = JText::_('PR PASSWORD NOT SAME');
       }elseif($fieldName == 'id_number'){
       	$langMsg = JText::_('PR INVALID NUMBER CARD');
       }elseif($fieldName == 'mobile_phone'){
       	$langMsg = JText::_('PR INVALID PHONE');
       }else{
       		$langMsg = JText::_('PR_FIELD_PR_INVALID_VALUE');
       }
	...
	function ajaxCheckUserName(){
	    ...
	    if(! empty($username))
	    {
		    ...
		    {
		    	$isInvalid	= true;
		    	$msg		= JText::_('PR IMPROPER USERNAME');
		    }
		}
	    ...
	    if(! empty($username) && !$isInvalid){
			...
			$msg = JText::sprintf('PR USERNAME EXIST', $username);
	    }
	...
	function ajaxCheckEmail(){
	    ...
	    $msg = JText::sprintf('PR EMAIL EXIST', $email);
