	...
	for( $i = 0 ; $i < count( $videos ); $i++ )
	{
		...
		switch($video->creator_type)
		{
			case VIDEO_GROUP_TYPE :
				$url				= 'index.php?option=com_contestants&view=videos&task=video&groupid='.$video->groupid.'&videoid='.$video->id;
				$video->url 		= CRoute::_( $url );
				$video->permalink	= CRoute::getExternalURL( $url , false );
				break;
			case VIDEO_USER_TYPE :
			default :
				$url				= 'index.php?option=com_contestants&view=videos&task=video&userid='.$video->creator.'&videoid='.$video->id;
				$video->url 		= CRoute::_( $url );
				$video->permalink	= CRoute::getExternalURL( $url , false );
				break;
		}
		...
		$video->isAdmin	= isContestantAdmin();
		...
		if( $video->lastupdated == '0000-00-00 00:00:00' || $video->lastupdated == '')
		{
			...
			{
				$video->lastupdated	= JText::_( 'MM NO LAST ACTIVITY' );
			}
			else
			{
...
// 		}
		...
		{
			$video->durationHMS = JText::_('MM VIDEO DURATION NOT AVAILABLE');
		}
...
function cGetVideoReturnUrlFromRequest($videoType='default')
{
	...
	{
		$defaultUrl	= CRoute::_('index.php?option=com_contestants&view=groups&task=viewgroup&groupid=' . $groupId , false );
		$pendingUrl	= CRoute::_('index.php?option=com_contestants&view=videos&task=mypendingvideos&userid='.$my->id.'&groupid='.$groupId, false);
		return ($videoType == 'pending') ? $pendingUrl : $defaultUrl;
	}

	$defaultUrl	= CRoute::_('index.php?option=com_contestants&view=videos&task=myvideos&userid=' . $my->id , false );
	$pendingUrl	= CRoute::_('index.php?option=com_contestants&view=videos&task=mypendingvideos&userid='.$my->id, false);
	return ($videoType == 'pending') ? $pendingUrl : $defaultUrl;
}
