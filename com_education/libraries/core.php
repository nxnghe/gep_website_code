...
class CFactory
{
	...
	function getUserFromTokenId( $tokenId , $userId )
	{
		...
		$query	= 'SELECT COUNT(*) '
				. 'FROM ' . $db->nameQuote( '#__contestants_photos_tokens') . ' '
	...
	function getUser($id=null, $obj = null)
	{
		...
		if( empty($instances[$id]) )
		{
			...
			{
				JError::raiseError( 500, JText::sprintf('MM CANNOT LOAD USER', $id) );
			}
			//echo "asign user here";exit;

			$instances[$id] = new CUser($id);
			//var_dump($instances[$id]);exit;
			...
			if($id == 0)
			{
				...
				$lang->load('com_education');

				$instances[$id]->name = JText::_('MM ACTIVITIES GUEST');
				$instances[$id]->username = JText::_('MM ACTIVITIES GUEST');
				$instances[$id]->_avatar 	= 'components/com_education/assets/default.jpg';
				$instances[$id]->_thumb 	= 'components/com_education/assets/default_thumb.jpg';
			}
		}
		//var_dump($instances[$id]);exit;
		return $instances[$id];
	...
	function getModel( $name = '', $prefix = '', $config = array() )
	{
		...
		if(!isset($modelInstances)){
			...
			include_once(JPATH_ROOT.DS.'components'.DS.'com_education'.DS.'libraries'.DS.'error.php');
		}

		if(!isset($modelInstances[$name.$prefix]))
		{
			include_once( JPATH_ROOT.DS.'components'.DS.'com_education'.DS.'models'.DS.'models.php');

			include_once( JPATH_ROOT.DS.'components'.DS.'com_education'.DS.'models'.DS. strtolower( $name ) .'.php');
			$classname = $prefix.'ContestantsModel'.$name;
	...
	function load( $type, $name )
	{
		...
		include_once(JPATH_ROOT.DS.'components'.DS.'com_education'.DS.$type.DS. strtolower($name) .'.php');
...
class CRoute
{
		...
		if(empty($itemid[$view])) {
			...
			if(!$isValid){
				$query  = "SELECT `id` FROM #__menu WHERE "
					." `link` LIKE '%option=com_education%' "
					." AND `published`='1' "
	...
	function _getViewItemid($view) {
		...
		if(empty($itemid[$view])){
			...
			$url = $db->quote('%option=com_education&view=' . $view . '%');
	...
	function _getDefaultItemid()
	{
		...
		$url = $db->quote("index.php?option=com_education&view=frontpage");
		...
		if(!$val)
		{
			$url = $db->quote("%option=com_education%");
...
class CUser extends JUser {
	...
	function delete()
	{
		...
		$query	= 'DELETE FROM ' . $db->nameQuote( '#__contestants_candidates' ) . ' '
		...
		if(!$this->_init) {
			...
			if($initObj == null ){
				$query  = "SELECT  "
						...
						. "	a.`thumb`	as _thumb , "
						. "s.`userid` as _isonline, u.* "
						. " FROM #__contestants_candidates as a "
						. " LEFT JOIN #__users u "
			 			. " ON u.`id`=a.`userid` "
						. " LEFT OUTER JOIN #__session s "
			 			. " ON s.`userid`=a.`userid` "
			...
			if(empty($obj))
			{
				if( !$obj && ($this->id != 0) )
				{
					...
					if( $isValid )
					{
						...
						$db->insertObject( '#__contestants_candidates' , $obj );
	...
	function getTooltip()
	{
		if(!$this->_tooltip)
		{
			require_once( JPATH_ROOT . DS . 'components' . DS . 'com_education' . DS . 'libraries' . DS . 'tooltip.php' );
...
class CAssets
{
	...
	function attach( $path, $type , $assetPath = ''){
		...
		{
			$path = JURI::root(). 'components/com_education'.$path;
		}
