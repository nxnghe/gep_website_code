...
/**
 * Templating system for Contestants
 */
class CTemplate {
    ...
    function _getTemplateFullpath($file)
    {
    	...
    	if(!JString::strpos($file, '.php'))
    	{
    		...
    		$overrideExists	= JFolder::exists( $overridePath . DS . 'com_contestants' );
			$template		= CONTESTANTS_COM_PATH . DS . 'templates' . DS . 'default' . DS . $filename . '.php';

    		// Test override path first
    		if( JFile::exists( $overridePath . DS . 'com_contestants' . DS . $filename . '.php') )
    		{
    			// Load the override template.
				$file	= $overridePath . DS . 'com_contestants' . DS . $filename . '.php';
			}
    		...
    		else
    		{
    			...
    			$file	= CONTESTANTS_COM_PATH . DS . 'templates' . DS . 'default' . DS . $filename . '.php';
			}
    	}
    	//echo $file;exit;
	...
	function addStylesheet( $file )
	{
		...
		if(!JString::strpos($file, '.css'))
		{
    		...
    		$overridePath	= JPATH_ROOT . DS . 'templates' . DS . $mainframe->getTemplate() . DS . 'html';
    		$overrideExists	= JFolder::exists( $overridePath . DS . 'com_contestants' );
			$template		= CONTESTANTS_COM_PATH . DS . 'templates' . DS . 'default' . DS . 'css' . DS . $filename . '.css';

    		// Test override path first
    		if( JFile::exists( $overridePath . DS . 'com_contestants' . DS . 'css' . DS . $filename . '.css') )
    		{
    			// Load the override template.
    			$file	= '/templates/' . $mainframe->getTemplate() . '/html/com_contestants/css/' . $filename . '.css';
			}
    		else if( JFile::exists( $template ) && !$overrideExists )
    		{
	   			// If override fails try the template set in config
				$file	=  '/components/com_contestants/templates/' . 'default' . '/css/' . $filename . '.css';
    		}
    		else
    		{
    			// We assume to use the default template
...
//
//     	$contents	= "";
// 		$file = CONTESTANTS_COM_PATH .DS. 'templates'.DS.$config->get('template').DS.$file . '.php';
//
//         if($this->is_cached()) {
