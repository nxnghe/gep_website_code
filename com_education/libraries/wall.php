...
class CWallLibrary
{
	...
	function saveWall( $uniqueId , $message , $appType , &$creator , $isOwner )
	{
		...
		$date = new JDate($date);
		$date = $date->toFormat(JText::_('%d/%m/%Y'));
	...
	function getWallContents( $appType , $uniqueId , $isOwner , $limit = 10 , $limitstart = 0, $templateFile = 'wall.content' )
	{
		...
		if( $walls )
		{
			...
			{
				$wall	=& $walls[ $i ];
				$user	= JFactory::getUser( $wall->post_by );
				//var_dump($user);//exit;
				//$date	= cGetDate( $wall->date );
				jimport('joomla.utilities.date');

				$wall->date = new JDate($wall->date);
				$wall->date = $wall->date->toFormat(JText::_('%d/%m/%Y'));
