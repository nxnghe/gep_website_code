 ...
 * @since		1.6
 */
class EducationModelTeacher extends JModelForm
{
	/**
	...
	public function register($data)
	{
      ...
      // var_dump($data);exit;
		...
		if ($userid > 0){
			$this->saveEditTeacher($userid, $data);
		}else{
			$this->saveTeacher($user->id, $data);
		}
	...
	function saveTeacher($userid, $fields){

		$db = JFactory::getDBO();

		//echo "i am at save teacher"; exit;
		...
		else
		{
			...
			else
			{
				...
				//@todo: configurable path for avatar storage?
				$storage			= JPATH_ROOT . DS . 'images' . DS . 'teachers';
				$storageImage		= $storage . DS . $hashFileName . cImageTypeToExt( $file['type'] );
				$storageThumbnail	= $storage . DS . 'thumb_' . $hashFileName . cImageTypeToExt( $file['type'] );
				$image				= 'images/teachers/' . $hashFileName . cImageTypeToExt( $file['type'] );
				$thumbnail			= 'images/teachers/' . 'thumb_' . $hashFileName . cImageTypeToExt( $file['type'] );
		...
		$query_new = "INSERT INTO #__teachers (userid, year, full_name, major, experiences, birth_day, gender, address, phone, province, email, about_me, avatar, thumb, published ) VALUES ("
	            	.$db->Quote($userid)
	            	. ', '.$db->Quote($curYear)
	            	. ', '.$db->Quote($full_name)
	               	. ', '.$db->Quote($major)
	               	. ', '.$db->Quote($experiences)
	            	. ', '.$db->Quote($birth_day)
	            	. ', '.$db->Quote($gender)
	            	. ', '.$db->Quote($address)
	            	. ', '.$db->Quote($phone)
    ...
    function saveEditTeacher($userid, $fields){
		...
		$about_me = $fields['about_me'];
		 ...
		 $query = "UPDATE #__teachers SET"
	            	...
	            	. ', `major`='.$db->Quote($major)
	            	. ', `experiences`='.$db->Quote($experiences)
