...
$birth_day = new JDate($this->teacherProfile[0]->birth_day);
...
<?php //var_dump($this->teacherProfile);
	//var_dump($this->teacherProfile[0]->avatar);
...
<div class="canDetail">
	...
	<table class="contestanttable" border="0">
		...
		<td class="contestantimagetd" style="text-align:left" >
			<img src="<?php echo ($this->teacherProfile[0]->avatar!='')?$this->teacherProfile[0]->avatar:'images/contestantsavatar/default_thumb.png';?>" width="250px;" />
			<?php
			/*
					if( isMine($this->teacherProfile[0]->userid, $my->id) || ( isContestantAdmin() ) ){
					?>
					<a href="<?php echo JURI::base().'index.php?option=com_contestants&view=register&layout=reavatar&task=edit&userid='.$teacherProfile->userid;?>" >
		...
		<td style="text-align: left;" class="contestantinfotd1">
			...
			<div class="leftvideo">
			<p class="contestantinfor" style="clear: both;"><span id="infobold1"><strong><?php echo $this->teacherProfile[0]->full_name;?></strong></span></p>
			<p class="contestantinfor" style="clear: both;"><span id="infobold">Birth Day: </span><?php echo $birth_day;?></p>
			<p class="contestantinfor"><span id="infobold">From:</span> <?php echo $this->teacherProfile[0]->province; ?></p>
			<p class="contestantinfor"><span id="infobold">Major:</span> <?php echo $this->teacherProfile[0]->major;?></p>
            <p class="contestantinfor"><span id="infobold">Experiences:</span> <?php echo $this->teacherProfile[0]->experiences;?></p>

			<div class="edit_profile_link1" style="float:right; clear:both">
			<?php
				if( isMine($this->teacherProfile[0]->userid, $my->id) || ( isContestantAdmin() ) ){
			?>
						<a href="<?php echo JURI::base().'index.php?option=com_education&view=form&layout=teacher&task=edit&userid='.$this->teacherProfile[0]->userid;?>" style="padding-right:15px!important">
							<span class="edit_teacherProfile"><?php echo JText::_('GEP EDIT PROFILE');?></span>
						</a>
						<a href="<?php echo JURI::base().'index.php?option=com_education&view=form&layout=class&userid='.$this->teacherProfile[0]->id;?>" style="padding-right:15px!important">
							<span class="edit_teacherProfile" style="padding-right:20px!important"><?php echo JText::_('GEP ADD CLASS');?></span>
						</a>
			<?php
			}
			?>
			...
			?>
							...
							<span class="edit_profile" onclick="mm.candidates.activeCandidadate('<?php echo $teacherProfile->userid; ?>');"><?php echo JText::_('MM ADD CANDIDATE');?></span>
		...
		<?php
		 ...
		 <div class="about_me_title">
			...
			<div class="about_me_text"><?php echo $this->teacherProfile[0]->about_me; ?></div>
				...
				<?php
							...
							//echo $comments;exit;
							  if (file_exists($comments)) {
								...
								echo JComments::showComments($this->teacherProfile[0]->id, 'com_education', ' Candidate ');
							  }
					?>
...
<?php if ( isMine($this->teacherProfile[0]->userid, $my->id) ) {
